...
# thirteen ranks (2 through 10, then the face cards Jack, Queen, King and Ace) for a total
# of 52 cards per deck. Jacks, Queens and Kings all have a rank of 10.
...
# You can develop your own system, or copy ours below. Finally, declare a Boolean value to be used to control while loops.
...
# A Card object really only needs two attributes: suit and rank. You might add an attribute
# for "value" - we chose to handle value later when developing our Hand class.
...
#
# OPTIONAL: We may never need to print the contents of the deck during gameplay, but having the ability to see the cards inside it may help troubleshoot any problems that occur during development.
...
# In addition to holding Card objects dealt from the Deck, the Hand class may be used to calculate the
# value of those cards using the values dictionary defined above. It may also need to adjust for the value of Aces when appropriate.
...
class Hand:
    ...
    def adjust_for_ace(self):
        ...
        while self.value > 21 and self.aces > 0:
            self.value -= 10
            self.aces -= 1
# We added code to the add_card method to bump self.aces whenever an ace is brought into the hand,
# and added code to the adjust_for_aces method that decreases the number of aces any time we make an adjustment to stay under 21.
...
# to check what these two cards are
